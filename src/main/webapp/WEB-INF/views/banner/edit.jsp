            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="box box-info">
                        ...
                        <form id="addForm" method="POST" action="${ctx}/banner/update" class="form-horizontal">
                            <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                            <input type="hidden" name="pkid" value="${banner.pkid}"/>
                            <div class="box-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10" id="imgContainer" >
                                        <c:if test="${banner.logoPkid != null && banner.logoPkid != ''}">
                                            <div class='thumbnail' style="text-align: left;">
                                                <input type='hidden' name='logoPkid' value="${banner.logoPkid}"/>
                                                <img src="${ctx}/resource/showImage?pkid=${banner.logoPkid}" style='height:180px;width: 400px;'/>
                                            </div>
                                        </c:if>
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control" name="name" id="name" value="${banner.name}" maxlength="50" placeholder="请输入名称"/>
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-4">
                                        <select name="bussinessType" id="bussinessType" class="form-control">
                                            <c:forEach var="item" items="${bussinessList}">
                                                <option value="${item.key}" <c:if test="${banner.bussinessType == item.key}">selected</c:if>>${item.value}</option>
                                            </c:forEach>
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-sm-4">
                                        <select name="forwardType" id="forwardType" class="form-control">
                                            <c:forEach var="item" items="${forwardList}">
                                                <option value="${item.key}" <c:if test="${banner.forwardType == item.key}">selected</c:if>>${item.value}</option>
                                            </c:forEach>
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control" name="url" id="url" value="${banner.url}" maxlength="500" placeholder="请输入跳转地址"/>
                                    </div>
                                </div>
                            </div>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $("#btn-submit").unbind("click").bind("click", function (event) {
            ZW.Ajax.doRequest("addForm", ctx + "/banner/update", "", function (data) {
                // 失败
                if (data.res == 0) {
