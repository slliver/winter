...
ZW = {
    ...
    Dict: {//ids 对应html中id值(自己命名，多个用逗号分隔)。keys 对应key值(数据库中的key，多个逗号分隔)。type(可选)1.请选择，2.自定义数组。默认不填.dfstr (可选)自定义数组
        setSelect: function (ids, keys, type, dfstr) {
            ...
            $.ajax({
                type: 'POST',
                headers: headers,
                url: ctx + '/sys/sysDict/getDictByKey',
                    ...
                    if (data.res == 1) {
                        ...
                        if (type == 1) {
                            ...
                            for (var i = 0; i < idss.length; i++) {
                                ...
                                opts = "";
                                $.each(map[idss[i]].items, function (n, v) {
                                    opts += "<option value='" + v.dictName + "'>" + v.dictName + "</option>";
                                });
                                $("#" + idss[i] + " select").append(opts);
                            }
                        }
    ...
    Page: {//跳转分页
        ...
        setSizeNew: function (formId, size, JpFun, pageNumNew) {
            $("#" + formId + " .pageNum").val(pageNumNew);
            $("#" + formId + " .pageSize").val(size);
            eval(JpFun + "()");
        },
        /*自定义跳转分页*/
        ...
        setSimPage: function (formId, pageId, pagesize, pagenum, totalCount, fun) {
            if (totalCount > 0) {
                ...
                if (pagenum - 1 >= 1) {
                    html += "<li class='prev'><a onclick='ZW.Page.jump(&apos;" + formId + "&apos;,1,&apos;" + fun + "&apos;)' href='#'>首</a></li>";
                }
                else {
                ...
                if (pagenum + 1 <= leng) {
                    html += "<li class='next'><a onclick='ZW.Page.jump(&apos;" + formId + "&apos;," + leng + ",&apos;" + fun + "&apos;)' href='#'>尾</a></li>";
                }
                else {
        ...
        setFunction: function (id, pBtn) {
            ...
            if (pBtn != null && pBtn.length > 0) {
                ...
                for (var i = 0; i < pBtn.length; i++) {
                    h += "<button type='button' class='btn btn-primary btn-xs' title='" + ZW.Object.notEmpty(pBtn[i].resourceName) + "' onclick='" + ZW.Object.notEmpty(pBtn[i].resourceFunType) + "(&apos;" + id + "&apos;,&apos;" + ZW.Object.notEmpty(pBtn[i].resourceUrl) + "&apos;,&apos;" + ZW.Object.notEmpty(pBtn[i].targetResourcePkid) + "&apos;)' class='btn btn-primary btn-xs" + ZW.Object.notEmpty(pBtn[i].icon) + "'>" + ZW.Object.notEmpty(pBtn[i].resourceName) + "</button>";
                }
                h += "</td>";
            } else {
                h += "<td></td>";
    ...
    Model: {//loading框
        ...
        check: function (id, title, fn) {
            $("#" + id).removeClass('hide').dialog({
                resizable: false,
                modal: true,
                title: "<div class='widget-header'><h4 class='smaller'>" + (ZW.Object.notNull(title) ? title : "查看") + "</h4></div>",
                title_html: true,
                buttons: [{
...
//                    		return;
    ...
    Validate: {//判断是否是英文数字,是返回true，不是返回false
        ...
        //判断是否合法的固定电话：
        isFixedPhone: function (phoneNum) {
            if (phoneNum.match(/^[+]{0,1}(\d){1,3}[ ]?([-]?((\d)|[ ]){1,12})+$/)) return true;
            return false;
        },
        //判断是否合法的固定电话：
        isMobilePhone: function (phoneNum) {
            if (phoneNum.match(/^(((13[0-9]{1})|(15[0-9]{1})|(18[0-9]{1}))+\d{8})$/)) return true;
            return false;
        },
        //最大长度的验证(最好使用maxLength属性即可，不用js验证了）。
        maxLength: function (val, minLen, maxLen) {
            min = parseInt(min);
            max = parseInt(max);
            if (val.match(/^\S{,20}$/)) {
                return true;
        ...
        minNumber: function (v, min) {
            v = parseInt(v);
            min = parseInt(min);
            if ((min <= v)) return true;
            return false;
        },
        maxNumber: function (v, max) {
            v = parseInt(v);
            max = parseInt(max);
            if ((max >= v)) return true;
            return false;
        },
        lessthanNumber: function (v, min) {
            v = parseInt(v);
            min = parseInt(min);
            if ((min < v)) return true;
            return false;
        },
        greaterThan: function (v, max) {
            v = parseInt(v);
            max = parseInt(max);
            if ((max > v)) return true;
            return false;
        ...
        decimals: function (v, min, max) {
            ...
            var num = v.length;
            if (v.indexOf(".") != -1) {
                if ((num <= maxNum) && (tmp[0].length <= max) && (tmp[1].length <= min)) {
                    return true;
                }
            } else {
        ...
        form: function (fromId, side) {
            ...
            $('#' + fromId + " input[zwValidate], #" + fromId + " select[zwValidate]").each(function () {
                if (res == false) return;
                var that = $(this);
                var zwValidate = $(this).attr("zwValidate").split(",");
                $.each(zwValidate, function (n, v) {
                    if (res == false) return;
                    if (v == 'required') {
                        if (!ZW.Object.notNull($.trim(that.val()))) {
                        ...
                        if (ZW.Object.notNull(that.val())) {
                            if (!ZW.Validate.isEmail(that.val())) {
                        ...
                        if (ZW.Object.notNull($.trim(that.val()))) {
                            if (!ZW.Validate.isDate(that.val())) {
                        ...
                        if (ZW.Object.notNull(that.val())) {
                            if (!ZW.Validate.isDatetime(that.val())) {
                        ...
                        if (ZW.Object.notNull(that.val())) {
                            ...
                            if (!ZW.Validate.numrange(that.val(), min, max)) {
                                that.tips({side: side, msg: "数字范围：" + min + "~" + max, bg: '#FF2D2D', time: 1});
                                that.focus();
                                res = false;
                            }
                        }
                        ...
                        if (ZW.Object.notNull(that.val())) {
                            var min = that.attr("min");
                            var max = that.attr("max");
                            if (!ZW.Validate.numrangeth(that.val(), min, max)) {
                                that.tips({side: side, msg: "数字范围：" + min + "~" + max, bg: '#FF2D2D', time: 1});
                                that.focus();
                                res = false;
                            }
                        ...
                        if (ZW.Object.notNull(that.val())) {
                            if (!ZW.Validate.isEn(that.val())) {
                                that.tips({side: side, msg: "只能输入英文", bg: '#FF2D2D', time: 1});
                                that.focus();
                                res = false;
                            }
                        }
                        ...
                        if (ZW.Object.notNull(that.val())) {
                            if (!ZW.Validate.isEnNum(that.val())) {
                        ...
                        if (ZW.Object.notNull(that.val())) {
                            if (!ZW.Validate.isNum(ZW.Comma.getValue(that.val()))) {
                    ...
                    else if (v == 'isFixedPhone') {
                        if (ZW.Object.notNull(that.val())) {
                            if (!ZW.Validate.isFixedPhone(that.val())) {
                                that.tips({side: side, msg: "只能输入固定电话号码", bg: '#FF2D2D', time: 1});
                                that.focus();
                                res = false;
                            }
                        }
                    }
                    else if (v == 'isMobilePhone') {
                        if (ZW.Object.notNull(that.val())) {
                            if (!ZW.Validate.isMobilePhone(that.val())) {
                                that.tips({side: side, msg: "只能输入英文或数字", bg: '#FF2D2D', time: 1});
                                that.focus();
                                res = false;
                            }
                        ...
                        if (ZW.Object.notNull(that.val())) {
                            var min = that.attr("minLen");
                            var max = that.attr("maxLen");
                            if (!ZW.Validate.maxLength(that.val(), min, max)) {
                                that.tips({side: side, msg: "只能输入英文或数字", bg: '#FF2D2D', time: 1});
                                that.focus();
                                res = false;
                            }
                        ...
                        if (ZW.Object.notNull(that.val())) {
                            var min = that.attr("min");
                            if (!ZW.Validate.minNumber(that.val(), min)) {
                                that.tips({side: side, msg: "最小值为：" + min, bg: '#FF2D2D', time: 1});
                                that.focus();
                                res = false;
                            }
                        }
                        ...
                        if (ZW.Object.notNull(that.val())) {
                            var max = that.attr("max");
                            if (!ZW.Validate.maxNumber(that.val(), max)) {
                                that.tips({side: side, msg: "最大值为：" + max, bg: '#FF2D2D', time: 1});
                                that.focus();
                                res = false;
                            }
                        }
                    }
                    else if (v == 'lessthanNumber') {
                        if (ZW.Object.notNull(that.val())) {
                            var min = that.attr("min");
                            if (!ZW.Validate.lessthanNumber(that.val(), min)) {
                                that.tips({side: side, msg: "最小值为：" + min, bg: '#FF2D2D', time: 1});
                                that.focus();
                                res = false;
                            }
                        ...
                        if (ZW.Object.notNull(that.val())) {
                            var max = that.attr("max");
                            if (!ZW.Validate.greaterThan(that.val(), max)) {
                                that.tips({side: side, msg: "最大值为：" + max, bg: '#FF2D2D', time: 1});
                                that.focus();
                                res = false;
                            }
                        ...
                        if (!ZW.Object.notNull(that.val())) {
                            ...
                            var min = that.attr("data-minDecimals");
                            var max = that.attr("data-maxDecimals");
                            if (!ZW.Validate.decimals(ZW.Comma.getValue(that.val()), min, max)) {
                                that.tips({
                                    side: side,
                                    msg: "小数点前最多为" + max + "位，小数点后最多为" + min + "位",
                                    bg: '#FF2D2D',
                                    time: 4
                                });
                                that.focus();
    ...
    Date: {//时间格式化(默认),time 时间
        Default: function (time) {
            return ZW.Object.notNull(time) ? (new Date(time).Format("yyyy-MM-dd")) : " ";
        },
        //时间格式化,time 时间,fmt 格式
        Format: function (time, fmt) {
            return ZW.Object.notNull(time) ? (new Date(time).Format(fmt)) : "";
        }
    },
    Numberic: {
        // 数值格式化，默认2位
        Default: function (num) {
            return ZW.Numberic.Format(num, 2);
        },
        // 数值格式化，可限定位数
    ...
    Money: {
        ...
        Default: function (money) {
            return ZW.Money.Format(money, 8);
        },
        //金额数值格式化，可限定位数
            ...
            $.ajax(
                {
                    ...
                    beforeSend: function (xhr) {
                        ...
                        if (ZW.Object.notNull(header)) {
                            xhr.setRequestHeader(header, token);
                        }
                        if (ZW.Object.notNull(comp)) {
                            comp.block({target: ".modal-content"});
                        }
                    },
                    complete: function () {
    ...
    File: {
        ...
        fileType: function (obj, aFmats) {
            if (ZW.Object.notNull(aFmats)) {
                ...
                $(obj).tips({side: 3, msg: '请上传' + aFmats + '格式的文件', bg: '#FF2D2D', time: 3});
                $(obj).val('');
            }
        }
    },
    Component: {
        ...
        setCascadeOptions: function (url, resName, type, dfstr, root) {
            $.ajax({
                type: 'POST',
                url: ctx + url,
                data: {resName: resName, type: type},
                dataType: 'json',
                success: function (data, textStatus) {
                    var map = data.obj;
                    var opts = "";
